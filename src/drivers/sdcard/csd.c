...
int sdGetCSD(MicronSdCardState *state, uint32_t timeout, void *csd) {
    ...
    return _sdWaitForData(state, csd, SD_CSD_SIZE, timeout);
}
...
int sdParseCSD(void *csdIn, SD_CSD *out) {
...
//XXX make this more user-friendly, less wasteful, and actually able to get
//more info than just the size.
...
int sdGetSize(MicronSdCardState *state, uint64_t *out, uint32_t timeout) {
    ...
    err = sdGetCSD(state, timeout, buf);
    if(err) return err;

    SD_CSD csd;
    err = sdParseCSD(buf, &csd);
    if(err) {
        #if SDCARD_DEBUG_PRINT
    ...
    #if SDCARD_DEBUG_PRINT
        printf("SD: Capacity: %llu bytes (", memoryCapacity);
        if(memoryCapacity / (1024ull * 1024ull * 1024ull) > 0) {
            printf("%lld Gbyte",
                memoryCapacity / (1024ull * 1024ull * 1024ull));
        }
        else if(memoryCapacity / (1024ull * 1024ull) > 0) {
            printf("%lld Mbyte", memoryCapacity / (1024ull * 1024ull));
        }
        else if (memoryCapacity / (1024ull) > 0) {
            printf("%lld Kbyte", memoryCapacity / (1024ull));
        }
        else printf("%lld byte", memoryCapacity);

        printf(", %llu sectors, %d bytes each)\r\n", blocks, blockSize);
    #endif //SDCARD_DEBUG_PRINT
